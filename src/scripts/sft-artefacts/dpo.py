...
class ScriptArguments:
    ...
    # Training parameters
    alpha: Optional[float] = field(
        default=0.1, metadata={"help": "The alpha parameter for DPO loss"}
    )
    model_name_or_path: Optional[str] = field(
        default="meta-llama/Llama-3.2-1B", metadata={"help": "The model name or path"}
    )
    dataset_path: Optional[str] = field(
        default="/path/to/dataset.csv",
        metadata={"help": "Path to the CSV dataset containing 'prompt', 'chosen', 'rejected'"},
    )
    max_length: Optional[int] = field(
        default=512, metadata={"help": "Maximum length of input tokens"}
    )
    max_prompt_length: Optional[int] = field(
        default=256, metadata={"help": "Maximum length of the prompt"}
    )
    max_target_length: Optional[int] = field(
        default=256, metadata={"help": "Maximum length of the response"}
    )
    torch_dtype: Optional[str] = field(
        default=None,
        metadata={
            "help": "Torch_dtype to use when loading and training the model (e.g., 'float16', 'bfloat16')"
        },
    )
    wandb_project: Optional[str] = field(
        default="dpo", metadata={"help": "Weights & Biases project name"}
    )
    wandb_run_name: Optional[str] = field(
        default="my_dpo_run", metadata={"help": "Weights & Biases run name"}
    )
...
if __name__ == "__main__":
    ...
    def process_sample(example):
        """
        Processes the dataset sample by tokenizing the prompt, chosen response, and rejected response.
        """
        ...
        example["prompt_tokens"] = tokenizer(prompt, truncation=True, max_length=script_args.max_prompt_length)
        example["chosen_tokens"] = tokenizer(chosen, truncation=True, max_length=script_args.max_target_length)
        example["rejected_tokens"] = tokenizer(rejected, truncation=True, max_length=script_args.max_target_length)
        return example

    dataset = dataset.map(process_sample, remove_columns=["prompt", "chosen", "rejected"])
