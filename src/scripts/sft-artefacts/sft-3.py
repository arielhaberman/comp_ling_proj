...
from transformers import (
    ...
    set_seed,
    TrainingArguments,
    DataCollatorForLanguageModeling  # Using a simpler collator
)
...
class ScriptArguments:
    ...
    model_name: Optional[str] = field(
        ...
        metadata={"help": "Name of the model to fine-tune"}
    )
    # dataset_name is not used since we load from a local file
    dataset_name: Optional[str] = field(
        default=None, 
        metadata={"help": "Name of the dataset (not used, local file is used)"}
    )
    dataset_path: Optional[str] = field(
        default="my_data.tsv", 
        metadata={"help": "Path to your local TSV/CSV file with prompt/original_response/llama_response"}
    )
    dataset_text_field: Optional[str] = field(
        default="full_text", 
        metadata={"help": "Column name of the concatenated prompt-response field"}
    )
    packing: Optional[bool] = field(
        default=False, metadata={"help": "Use dataset packing if True."}
    )
    append_concat_token: Optional[bool] = field(
        default=False, metadata={"help": "Append eos_token_id at the end of each sample."}
    )
    add_special_tokens: Optional[bool] = field(
        default=False, metadata={"help": "Add special tokens to each sample if True."}
    )
    eval_sample_size: Optional[int] = field(
        default=None, metadata={"help": "Sub-sample size for evaluation dataset if needed."}
    )
    preprocessed_data_dir: Optional[str] = field(
        default=None, metadata={"help": "If preprocessed data exists, but we ignore it here."}
    )
    wandb_project: Optional[str] = field(
        default="sft", metadata={"help": "Weights & Biases project name."}
    )
    wandb_run_name: Optional[str] = field(
        default="my_sft_run", metadata={"help": "W&B run name"}
    )
    sanity_check: Optional[bool] = field(
        default=False, metadata={"help": "If True, train on only a subset (e.g. 1000 samples)."}
    )
    seq_length: Optional[int] = field(
        default=512, metadata={"help": "Maximum input sequence length."}
    )
    load_in_8bit: Optional[bool] = field(
        default=False, metadata={"help": "Load the model in 8-bit precision if True."}
    )
    load_in_4bit: Optional[bool] = field(
        default=False, metadata={"help": "Load the model in 4-bit precision if True."}
    )
    trust_remote_code: Optional[bool] = field(
        default=True, metadata={"help": "Trust remote code needed for some Llama models."}
    )
    peft_lora_r: Optional[int] = field(
        default=64, metadata={"help": "LoRA r parameter"}
    )
    peft_lora_alpha: Optional[int] = field(
        default=16, metadata={"help": "LoRA alpha parameter"}
    )
    use_reentrant: Optional[bool] = field(
        default=False, metadata={"help": "Use re-entrant gradient checkpointing if True."}
    )
    use_flash_attn: Optional[bool] = field(
        default=False, metadata={"help": "Use flash attention if True."}
    )
    torch_dtype: Optional[str] = field(
        default=None,
        metadata={"help": "Torch dtype (e.g. 'float16','bfloat16', etc.)"}
    )
...
# Ensure no evaluation since we have no val dataset
...
# Step 1: Load the model
if script_args.load_in_8bit and script_args.load_in_4bit:
    raise ValueError("Cannot load model in both 8-bit and 4-bit simultaneously.")
elif script_args.load_in_8bit or script_args.load_in_4bit:
    quantization_config = BitsAndBytesConfig(
...
# Assuming your file is CSV/TSV. If TSV, specify delimiter="\t".
# If CSV with commas, remove delimiter or set delimiter=",".
dataset = load_dataset("csv", data_files={"train": script_args.dataset_path}, delimiter="\t")

if script_args.sanity_check:
    dataset["train"] = dataset["train"].shuffle(seed=42).select(range(1000))

def concat_prompt_and_response(sample):
    sample["full_text"] = f"Human: {sample['prompt']}\nAssistant: {sample['original_response']}"
    return sample

dataset["train"] = dataset["train"].map(concat_prompt_and_response)

training_dataset = dataset["train"]
val_dataset = None
test_dataset = None

# Use a simpler LM collator
...
trainer = SFTTrainer(
    ...
    max_seq_length=script_args.seq_length,
    train_dataset=training_dataset,
    dataset_text_field= "full_text", #script_args.dataset_text_field,  # "full_text"
    data_collator=collator,
    eval_dataset=val_dataset,
    packing=script_args.packing,
    dataset_kwargs={
